...
object FcmAlertManager {
    ...
    fun sendHighSeverityAlert(context: Context) {
        val uid = FirebaseAuth.getInstance().currentUser?.uid ?: run {
            Log.w(TAG, "No authenticated user, skipping alert")
            return
        }

        DeviceRepository.getFid { fid ->
            if (fid == null) {
                Log.w(TAG, "FID unavailable, skipping alert")
                return@getFid
            }
            ...
            thread(name = "FcmAlert") {
                try {
                    ...
                    connection.apply {
                        ...
                        setRequestProperty("X-Oversee-Secret", ALERT_SECRET)
                        doOutput = true
                        connectTimeout = 10_000
                        readTimeout = 10_000
                    }

                    val body = JSONObject().put("uid", uid).put("child_fid", fid).toString()
